    ...
    <div class="content">
        ...
        <div class="panel panel-flat">
            <div class="panel-heading">
                <h5 class="panel-title">قابلیت ها</h5>
                <div class="heading-elements">
                    <ul class="icons-list">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tr class="border-double">
                        <th>ردیف</th>
                        <th scope="col">نام قابلیت</th>
                        <th scope="col">نام نمایشی قابلیت</th>
                        <th scope="col">عملیات ها</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($abilities as $abilitiess)
                        <tr>
                            <th scope="row">{{$abilitiess->id}}</th>
                            <td>{{$abilitiess->name}}</td>
                            <td>{{$abilitiess->title}}</td>
                            <td>
                                <div class="btn-group inline-display" role="group" aria-label="Basic example">
                                    <a  href="{{ URL::to('admin/products/' . $abilitiess->id . '/edit') }}">
                                        <button type="button" class="btn btn-primary">ویراییش</button>
                                    </a>&nbsp;
                                    <form style="display: inline;" action="{{url('admin/products', [$abilitiess->id])}}" method="POST">
